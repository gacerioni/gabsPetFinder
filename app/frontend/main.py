...
# Find Pet
    ...
    if st.button("Find Pet"):
        if pet_image:
            # Send request to backend
            files = {"file": pet_image.read()}  # Read the file as binary
            response = requests.post(f"{API_BASE_URL}/pets/find/", files=files)
            if response.status_code == 200:
                result = response.json()
                if "id" in result:
                    # Display pet information
                    st.success(f"🎉 Pet Found: {result.get('name', 'Unknown')} ({result.get('species', 'Unknown')}, {result.get('breed', 'Unknown')})")
                    st.image(result["image_url"], caption=f"{result.get('name', 'Unknown')} ({result.get('species', 'Unknown')})", use_container_width=True)
